                                ...
                                esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            ...
            ESP_LOGI(TAG,
                     "app regist failed: app_id %04x, status %d\n",
                                        ...
                                        esp_ble_gatts_cb_param_t *param)
{
    ...
    uint8_t res = find_gatts_index(param->write.handle);
    ...
    case ESP_GATTS_READ_EVT:
        res = find_gatts_index(p_data->read.handle);
